...
#define directory 	"/Users/vishrut009/Documents/6th Sem/IRE/P2/Final_2/mini_project/"
...
void read_title_sec_index()
{
	FILE *f;
	strcpy(path,directory);
	strcat(path,"chunks/title_id_secondary_index");
	f = fopen(path,"r");
	LL offset,id;
	while(fscanf(f,"%s %lld",word,&offset)!=EOF)
	{
		id = convert(word); // convert word back to the id 
...
void read_index_sec_index()
{
	FILE *f;
	strcpy(path,directory);
	strcat(path,"chunks/secondary_index");
	f = fopen(path,"r");
	LL offset;
	while(fscanf(f,"%s %lld",word,&offset)!=EOF)
	{
...
{
	map_tags();
	read_stop_words(); // get stop words
	read_title_sec_index();
	read_index_sec_index();
	strcpy(path,directory);
	strcat(path,"chunks/INDEX");
	F = fopen(path,"r");
	strcpy(path,directory);
	strcat(path,"chunks/TITLE-ID");
	T = fopen(path,"r");
	char ch[] = "Y";
	while(strcmp(ch,"Y")==0)
	{
